#!/bin/bash
# Deploy rag_retriever Lambda
# Run from project root: bash src/lambda/rag_retriever/deploy.sh

set -e

FUNCTION_NAME="rag_retriever"
LAMBDA_DIR="src/lambda/rag_retriever"
ZIP_FILE="rag_retriever.zip"
REGION="eu-west-1"
...
echo "=== Setting environment variables (update) ==="
# Use JSON via Python to safely handle passwords with special characters (@, !, # etc.)
ENV_JSON=$(python3 -c "
import os, json
print(json.dumps({
